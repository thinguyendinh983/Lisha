...
namespace Lisha.Infrastructure.Persistence.Context
{
    public abstract class BaseDbContext : IdentityDbContext<ApplicationUser, ApplicationRole, string, IdentityUserClaim<string>, IdentityUserRole<string>, IdentityUserLogin<string>, ApplicationRoleClaim, IdentityUserToken<string>>
    {
        ...
        protected BaseDbContext(DbContextOptions options, ICurrentUser currentUser, ISerializerService serializer, IOptions<DatabaseSettings> dbSettings, IEventPublisher events, IDateTime dateTime)
            : base(options)
        {
        ...
        private List<AuditTrail> HandleAuditingBeforeSaveChanges(Guid userId)
        {
            ...
            foreach (var entry in ChangeTracker.Entries<IAuditableEntity>()
                ...
                .ToList())
            {
                var trailEntry = new AuditTrail(entry, _serializer, _dateTime)
                {
